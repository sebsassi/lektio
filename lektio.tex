...
\tikzset{
    ...
    path fading on/.code args = {<#1>#2}{
        \alt<#1>{\pgfkeysalso{path fading = #2}}{\pgfkeysalso{opacity = 0}}
    }
}
    ...
    \begin{tikzpicture}[
        ...
        \begin{scope}[visible on = <3->]
            \node[inner sep = 0pt] at (sun) {
                \includegraphics[width = 1.4cm]{alt_sun_transparent_bright.png}};
...
\begin{frame}
    ...
    \begin{tikzpicture}[
        ...
        \begin{scope}
            ...
            \fill[
                    yellow!80!white,
                    path fading on = <2->electron cloud]
                (atom2) circle [radius=3.0cm];
            \fill[
                    yellow!80!white,
                    path fading on = <3->electron cloud]
                (atom12) circle [radius=3.0cm];
            \fill[
                    yellow!80!white,
                    path fading on = <2>electron cloud]
                (atom11) circle [radius=3.0cm];
            
            % nuclei
            \node[circle, minimum size = 1.0cm]
                (atom11 node) at (atom11) {}
                pic [
                        visible on = <1-2>,
                        shift = {(atom11)},
                        scale = 0.4] {
            ...
            % annotations
            \node[visible on = <2>]
                at ($(atom11) + (0,-2)$) [font = \scriptsize] {
                    elektronipilvi};
            \node[visible on = <3->]
                at ($(atom12) + (0,-2)$) [font = \scriptsize] {
                    elektronipilvi};
            \node[visible on = <1-2>]
                at (atom11) [
                        above = 0.3cm,
            ...
            \draw[visible on = <1->, scatter arrow]
                ($(atom11) + (-145:3.0cm)$) -- (atom11 node.west) -- (dm);
            \draw[visible on = <1-2>, scatter arrow]
                (atom11 node.east) -- (atom2 node.west);
            \draw[visible on = <3->, scatter arrow]
                (atom12 node.east) -- (atom2 node.west);
            
            \coordinate
                (ionize base) at ($(atom12) + (1.5,0.4)$);
            \node[visible on = <4->, circle, minimum size=0.3cm]
                (ionize) at ($(ionize base) + (80:1)$) {};
            \pic at (ionize) [visible on = <4->, very thick, scale = 0.4] {
                    electron};
            \draw[visible on = <4->, scatter arrow]
                (ionize base) -- (ionize);
            \node[visible on = <4->]
                at (ionize) [above right, font = \scriptsize] {
                    elektroni};
            
            \coordinate
                (light base) at ($(atom12) + (1.4,-0.4)$);
